...
async function displayStoreProducts() {
    ...
    try {
        // Make an API request to fetch store-specific product datahttps://flashweb.iweberp.com/api/kj_store_products?storeId=$storeId
        const response = await fetch(`https://flashweb.iweberp.com/api/kj_store_products?storeId=${storeId}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
        const data = await response.json();
        // Display the fetched data
        if (data && data.KJHyperLocalProduct && data.KJHyperLocalProduct.length > 0) {
            productList.innerHTML = '<h2 class="col-12 text-center d-none">Products:</h2>';
            data.KJHyperLocalProduct.forEach(product => {
                const productDiv = document.createElement('div');
                productDiv.classList.add('product');
                productDiv.classList.add('col');
                // console.log(product)
                const off = ((product.productMrp - product.productPrice) / product.productMrp) * 100
                // console.log(off.toFixed(0),product.productMrp.toFixed(2),product.productPrice.toFixed(2))
                productDiv.innerHTML = `    
        <div class="card h-100 border-0 contact-card">
            <img src="${product.productImage}" class="card-img-top" height="200px" alt="${product.productName}">
            <div class="card-body  text-center pb-0">
              <h5 class="card-title text-white fw-bold mb-3">${product.productName}</h5>
              <h5 class="card-title text-white fw-bold off_price"><span style="color:#F34333;" class="fs-6 fw-normal">${off.toFixed(0)}% off </span>₹${product.productPrice.toFixed(2)}</h5>
                <div class="d-flex align-items-center justify-content-center">
                    <p class=" text-muted mb-2 mrp">MRP -&nbsp;</p>
                    <p class=" text-muted text-decoration-line-through mb-2 mrp"> ₹${product.productMrp.toFixed(2)}</p>
                </div>
                <p class="small card-description text-white-50">${product.productDescription}</p>
            </div>
            <div class="card-footer border-0 bg-transparent text-center pt-0 pb-4">
                <a href="#" id="grabDealLink" data-bs-toggle="modal" data-bs-target="#exampleModal" class="btn shadow rounded-5 border-0 text-white py-1 px-4 fw-bold mx-auto ${product.productName}" style="background-color: #2e3192;">Grab This Deal</a>
            </div>
        </div>
                ...
                if (
                    off.toFixed(0) <= 0 ||
                    isNaN(off) ||
                    isNaN(product.productMrp) ||
                    isNaN(product.productPrice) ||
                    parseFloat(product.productMrp.toFixed(2)) <= 0 ||
                ...
                if (grabDealLink) {
                    ...
                    gtag('event', 'KJ_QR_click_web', {
                      'KJ_QR_click_web_category': 'Link Click',
                      'KJ_QR_click_label': `Product Name:${productData.productName},${productData.storeId} User No:${mobileNumber} User Name:${fName}+${lName}`,
